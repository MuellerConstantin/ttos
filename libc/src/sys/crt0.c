 ...
 * top, directly followed by the argv pointer array (argv[0] .. argv[argc - 1], NULL) and the envp
 * pointer array (envp[0] .. envp[envc - 1], NULL) right behind it. This stub reads them off the
 * stack, publishes the environment, calls main, and exits with its return value.
 *
...
__attribute__((naked, noreturn)) void _start(void) {
    __asm__(
        ...
        "lea 8(%esp,%eax,4), %ecx\n"   // envp = argv + argc + 1 (skips the argv NULL terminator)
        "mov %ecx, environ\n"
        "push %ecx\n"                  // envp
        "push %ebx\n"                  // argv
        "push %eax\n"                  // argc
